import React from 'react';
import { motion } from 'framer-motion';
import { Card } from '../ui/Card';
import { ShieldCheck, Activity, Gauge, Clock, Sliders, TrendingUp, HeartPulse } from 'lucide-react';

export const TrafficIntelligenceCards: React.FC = () => {
  const metrics = [
    {
      title: 'City Traffic Score',
      value: '84 / 100',
      change: 'Optimal City Flow',
      isPositive: true,
      icon: ShieldCheck,
      gradient: 'from-slate-900 via-emerald-950/70 to-slate-900 border-emerald-500/30',
      iconBg: 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30',
    },
    {
      title: "Today's Performance",
      value: '+6.2%',
      change: 'Flow Efficiency Index',
      isPositive: true,
      icon: Activity,
      gradient: 'from-slate-900 via-brand-950/70 to-slate-900 border-brand-500/30',
      iconBg: 'bg-brand-500/20 text-brand-400 border border-brand-500/30',
    },
    {
      title: 'Congestion Index',
      value: '28.4%',
      change: '-3.1% vs Yesterday',
      isPositive: true,
      icon: Gauge,
      gradient: 'from-slate-900 via-sky-950/70 to-slate-900 border-sky-500/30',
      iconBg: 'bg-sky-500/20 text-sky-400 border border-sky-500/30',
    },
    {
      title: 'Average Travel Time',
      value: '14.2 min',
      change: '-2.4 min Corridor Save',
      isPositive: true,
      icon: Clock,
      gradient: 'from-slate-900 via-cyan-950/70 to-slate-900 border-cyan-500/30',
      iconBg: 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30',
    },
    {
      title: 'Signal Efficiency',
      value: '96.4%',
      change: 'AI Green Wave Active',
      isPositive: true,
      icon: Sliders,
      gradient: 'from-slate-900 via-purple-950/70 to-slate-900 border-purple-500/30',
      iconBg: 'bg-purple-500/20 text-purple-400 border border-purple-500/30',
    },
    {
      title: 'Vehicle Growth',
      value: '+18.4%',
      change: 'vs Previous Month',
      isPositive: true,
      icon: TrendingUp,
      gradient: 'from-slate-900 via-amber-950/70 to-slate-900 border-amber-500/30',
      iconBg: 'bg-amber-500/20 text-amber-400 border border-amber-500/30',
    },
    {
      title: 'Traffic Health Score',
      value: '92 / 100',
      change: 'Excellent Network',
      isPositive: true,
      icon: HeartPulse,
      gradient: 'from-slate-900 via-teal-950/70 to-slate-900 border-teal-500/30',
      iconBg: 'bg-teal-500/20 text-teal-400 border border-teal-500/30',
    },
  ];

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
      {metrics.map((card, idx) => {
        const Icon = card.icon;
        return (
          <motion.div
            key={card.title}
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.25, delay: idx * 0.04 }}
          >
            <Card
              className={`p-4 bg-gradient-to-br ${card.gradient} border backdrop-blur-xl shadow-xl hover:scale-[1.02] transition-all duration-300 rounded-2xl group cursor-pointer flex flex-col justify-between h-full`}
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    {card.title}
                  </p>
                  <h3 className="text-xl font-black text-white font-mono tracking-tight mt-1">
                    {card.value}
                  </h3>
                </div>
                <div className={`p-2 rounded-xl ${card.iconBg} shrink-0`}>
                  <Icon className="w-4 h-4" />
                </div>
              </div>

              <div className="mt-3">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-950/80 text-slate-300 border border-slate-800 font-mono inline-block">
                  {card.change}
                </span>
              </div>
            </Card>
          </motion.div>
        );
      })}
    </div>
  );
};
